...
function getFarmaciaAbiertaStatus(
  id,
  morningOpening,
  morningClosing,
  lateOpening,
  ...
  if (isFarmaciaGuardia(id)) {
    ...
    if (timeOut <= 70) {
      ...
      farmaciaAbierta.time =
        ", cierra en " + Math.round(timeOutMinutes) + " minutos";
    }
    ...
    if (
      dateActual >= dateNumber.morningClosing &&
      dateActual < dateNumber.lateClosing
    ) {
...
class FarmaciasCol extends Component {
  ...
  render() {
    ...
    if (this.state.lateOpening !== "" && this.state.lateClosing !== "") {
      hourLate =
        " y " + this.state.lateOpening + " a " + this.state.lateClosing;
    }
      ...
      <div className="farmacia__item">
        <div className="farmacia__content">
          ...
          <p>
            <span
              className="c-tag"
              style={{ backgroundColor: this.state.farmaciaAbierta.color }}
            >
...
class FarmaciasStatus extends Component {
  render() {
    var text;
    if (
      this.props.status === "open-morning" ||
      this.props.status === "open-late"
    ) {
      ...
      return <span className="c-tag__text">{text}</span>;
    } else {
      return false;
    }
  }
